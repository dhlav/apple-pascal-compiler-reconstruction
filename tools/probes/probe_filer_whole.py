"""SYSTEM.FILER, the whole file, byte for byte.

src/pascal/programs/1.3/FILER.text compiled by Apple's compiler
(acceptance/2026-09-12-filer-complete), then slot 1 copied into a fresh
codefile by Apple's LIBRARY.CODE with the copyright notice -- the release
step finding 267 found for every system program.

Claims, each of which the binary can fail:

  1. **LIBFILER.CODE equals shipped SYSTEM.FILER in all 15360 bytes.**
     A copy with one byte changed must be caught, or the comparison is
     not looking.
  2. **Slot 1 is the reconstruction's bytes**: the compile's FILEHAND
     segment, the Librarian's and Apple's are the same 14646 bytes, and
     the segment holds all 56 procedures.
  3. **The Librarian did its part**: the compile alone is not Apple's
     file -- PASCALSY in slot 0, no notice.
  4. **The ancestor is the one recorded**: the UCSD II.0 Filer source in
     evidence/reference/ucsd-ii0-filer still has the hashes its README
     entry gives, so finding 271's comparisons stay checkable.
  5. **The verified source is the source in the tree**: the FILER.text
...
RUN = ROOT / "acceptance" / "2026-09-12-filer-librarian" / "LIBFILER.CODE"
COMPILE = ROOT / "acceptance" / "2026-09-12-filer-complete"
...
def main() -> int:
    ...
    check(len(ours) == len(apple) == 15360,
          f"both are {len(apple)} bytes, 30 blocks (ours {len(ours)})")
    ...
    check(len(mine) == SEGLEN and segment(ours, 1) == mine
          == segment(apple, 1),
          f"FILEHAND: compile == Librarian output == shipped "
          f"({len(mine)} bytes)")
    ...
    check(compiled != apple, "FILER13.CODE differs from SYSTEM.FILER")
