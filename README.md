...
evidence/          original inputs, never modified
  ...
  disks/           both complete Apple Pascal disk sets, 1.1 and 1.3:


                   APPLE1 (boot, with SYSTEM.APPLE -- the interpreter),


                   APPLE2 (SYSTEM.COMPILER) and APPLE3 (utilities).


                   The APPLE0 variants are excluded, see finding 47c.
...
src/               reconstructed source, hand-written
  ...
  pascal/programs/1.3/  the APPLE3 utilities. LINEFEED is closed:
                   Apple's own 1.1 source, recompiled by Apple's 1.3
                   compiler to the shipped bytes (finding 99).


  native/         6502 in the Apple Pascal Assembler's language, all of it

                   reassembling to Apple's exact bytes, relocation tables

                   included (findings 45, 98): SEARCH.TEXT is 1.3's

                   IDSEARCH and TREESEARCH, and APPLESTF/TURTLEGR/LONGINTS

                   are the library's fourteen native procedures.
...
| `LONGINTIO` | 30 | 3 of 3 p-code, plus 1852 bytes of 6502 |

| `APPLESTUFF` | 22 | 2 of 2 p-code, plus 584 bytes of 6502 |

| `TURTLEGRAPHICS` | 20 (+ 21) | 24 of 24 p-code, the 386-byte data segment, plus 2246 bytes of 6502 |



Every p-code figure there is Apple's own compiler running under the emulator

and the output compared byte for byte. The 6502 is the other half: three of

the six units are part assembly, the *linker* puts it in rather than the

compiler, and `src/native/` now carries all fourteen of those procedures as

source that assembles to the shipped bytes -- relocation tables and all

(finding 98). So all six units are closed end to end: 55 p-code procedures

and 14 native ones, every byte accounted for.



`SYSTEM.COMPILER` is not closed the same way, for the same structural reason

in reverse: 948 bytes of it arrive through the linker, and running `L(ink`

under the emulator is what would settle them (finding 91).
