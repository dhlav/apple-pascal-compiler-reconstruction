...
| BINDER | Joins assembly language parts into Pascal programs. | **reconstructed** (`src/pascal/programs/1.3/BINDER.text`, finding 117), no shipped source existed anywhere on the evidence disks. 6 procedures, compiled clean under Apple's own `SYSTEM.COMPILER` after a handful of real fixes; five of six procedures' frames exact, the outer program body short by one word out of 1119 (documented, not forced). A 1.1-vintage binary Apple's 1.3 disk never rebuilt (finding 99c); confirmed it runs under 1.3 off `SYSHD:`, so it's in scope |
