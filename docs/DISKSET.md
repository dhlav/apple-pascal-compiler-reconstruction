...
| LIBMAP.CODE | Shows you what is inside a library file. | **in progress**; 12 procedures. Native half free (finding 109 -- `LIBMAP.2` is Apple's own `IDSEARCH`, reused whole from `SYSTEM.COMPILER`); `SWAPBYTES`/`VALIDNAME` verified byte-for-byte, `SHOWSEGS` exact on param/data size, `SWAPALL` close (one word of locals short, documented not forced), `NEEDSSWAP` two words short (its wrong parameter type fixed via a real call site, finding 120, but the frame itself was already close before that); `SHOWONE`'s entry-reading loop and `GETWORD` reconstructed instruction-for-instruction and **acceptance-tier verified exact** (`params=2/data=556` and `params=2/data=2`, finding 111 update); the outer block **exact** (`params=4/data=1510`, finding 120); `SHOWREF` and `MAPLIBRARY` each one word short (documented not forced, finding 120); `SHOWINFO` (the largest procedure in the file, 1042 words of locals) is the one stub left |
