...
| LIBRARY.CODE | Utility to add or remove tools from library files. | **in progress**; 16 procedures. Compiles clean under Apple's own compiler with the right structure (finding 113), and under this project's own reconstructed compiler byte-identically to Apple's (finding 114). `SWAPBYTES`/`CHECKIO`/`GETINPUT`/`COPYSLOT`/`GETCOMMAND` frames exact; `NEEDSSWAP`/`SWAPALL`/`MSGLINE`/`MSGLINEINT`/`SHOWDICT` close (a few words short, documented); `MAINLOOP` has an unexplained 74-word gap; `COPYSLOT` (finding 115) has its real MARK/NEW/RELEASE segment-copy logic written and acceptance-verified exact; `COPYLINK`/`READLINK`/`COPYINTERFACE` still stubs |
